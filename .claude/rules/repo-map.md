...
| `Other/investor-skills/` | An imported third-party pack (88 persona-based investor judgment skills — `buffett`, `soros-reflexivity`, `turtle-trading`, crypto/VC/forensic-short/activist ones, etc. — the "Questflow" `SKILL.md` + `invest.md` format, plus its own `docs/`, `examples/`, `template/`, `poster-scripts/`). Deliberately **outside** `.claude/skills/` so Claude Code never auto-discovers or fires any of them — staged here only so the user can hand-pick individual skills to upload elsewhere (e.g. ChatGPT). Not integrated into this catalog: no README/backlog rows, no reciprocal carve-outs against `Finance/`, not part of `skill-interaction-testing`'s pool. Each file authors an investor's own verdict from Claude's judgment — the opposite operating assumption from every `Finance/` gate, which withholds an answer until the user supplies real, sourced numbers. If a specific persona's logic is ever wanted as a real part of this catalog, graduate it deliberately through `adding-a-skill.md`, one skill at a time — do not bulk-integrate. |
