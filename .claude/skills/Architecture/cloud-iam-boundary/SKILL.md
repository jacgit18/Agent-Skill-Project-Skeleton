...
Follow-ups:          <execution model this role attaches to → serverless-execution-model; edge/DDoS protection if internet-facing → resilience-strategy; encrypted transport between services (mTLS) → service-mesh-adoption; other encryption/secrets → name and defer; audit alerting → observability-strategy>
```
