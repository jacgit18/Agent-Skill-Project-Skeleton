...
- **Query-level and index tuning** of one slow query — the missing index, the N+1, the unbounded scan → `relational-modeling` (index/key design) or `problem-solving-gates` (Rubber Duck). This is frequently the *real* fix and framework step 1 sends you there first.
