...
description: |-
  Decide in advance, as numbers, which stocks are worth researching — the filter before a name reaches `equity-research-writeup` — or run one through it. Triggers: "set up screening criteria", "what filters should I use", "I keep hearing names from YouTube", "does TICKER pass my screen". Two modes. Define — the user commits a screen: a stated style (value / quality / growth / momentum), a small set of criteria (metric + threshold + direction + why it predicts returns) and a hard-disqualifier list; Claude checks each is numeric and coheres. Screen — a ticker is run against an existing screen: the user brings metric values from a real source (Claude does not fetch them) and gets pass/fail per line, a verdict, and a flag when a narrative-sourced candidate fails. No screen yet: Screen gates back to Define. Not `equity-trade-decision` (the next stage), not `portfolio-thesis-audit` or `position-exit-rules`, not a stock-picking service, not ETFs (`etf-selection`). `stock-review-order` names this as step 1's owner.
---
