...
description: |-
  Decide, as numbers, which stocks are worth researching — the filter before `equity-research-writeup` — or run a candidate through it. Triggers: "set up screening criteria", "what filters should I use", "I keep hearing names from YouTube", "does TICKER pass my screen". Two modes. Define — the user commits a screen: a stated style (value / quality / growth / momentum), a small set of criteria (metric + threshold + direction + why it predicts returns) and a hard-disqualifier list; Claude checks each is numeric and coheres. Screen — a ticker is run against an existing screen: the user brings metric values from a real source (Claude does not fetch them) and gets pass/fail per line, a verdict, and a flag when a narrative-sourced candidate fails. No screen yet: Screen gates back to Define. Not `equity-trade-decision`, `portfolio-thesis-audit`, `position-exit-rules`, or ETFs (`etf-selection`). `stock-review-order` names this as step 1's owner; filing a verdict to a real Webull watchlist is `watchlist-screen-sync`.
---
...
- **Portfolio construction.** How many names, sector limits, position sizing — not here;
  that's `asset-allocation-policy`.
...
Verdict:  <ADD TO WATCHLIST — clears the screen, proceed to `equity-research-writeup` for the
          full thesis, then `equity-trade-decision` | REJECT — fails <k> criteria / a
          disqualifier | INCOMPLETE — <m> metric values not supplied, get them first>
...
Feeds `equity-research-writeup` (the full thesis) → then `equity-trade-decision` — a name that
clears the screen is one worth that research effort.
