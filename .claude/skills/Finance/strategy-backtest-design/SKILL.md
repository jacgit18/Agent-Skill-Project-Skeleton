...
1. **Ticker(s)** — a real symbol, named directly or pulled from a Webull watchlist (e.g.
   "Screen Passed", one of the five "AI [Style]" watchlists, AI Shortlist, or AI Day
   Trade — all populated via `watchlist-screen-sync`, including its Candidate discovery
   mode).
