---
name: etf-selection
description: Use when someone is choosing, comparing, or re-checking a specific ETF or index fund by ticker — "should I hold SPY or VOO", "is this AI / cybersecurity / clean-energy ETF worth adding", "which bond ETF for my fixed-income sleeve", "check the ETFs I already own", "VTI vs a total-world fund". It is a gate: it withholds a hold/buy/swap verdict until the user has (a) named which allocation sleeve the fund fills — from `asset-allocation-policy` — and (b) pulled the fund's actual facts from the issuer fact sheet or prospectus, not from memory: expense ratio, the index tracked and what it actually holds (number of holdings, top-10 weight, sector/geography), AUM and average daily volume, tracking difference vs. the index over multiple years, structure (physical vs. synthetic, in-kind creation for tax efficiency, domicile), distribution yield and whether it is return-of-capital, and fund age — plus, for a bond fund, duration / credit quality / yield-to-maturity, and for a commodity fund, physically-backed vs. futures-based (roll drag) and the tax treatment. It also forces a holdings-overlap check against every fund already held that this would sit beside (a new fund that is 85% the same names is a fee-heavy duplicate or an undisclosed concentrated tilt), and — for any non-core sector / factor / thematic fund — a stated why-this-tilt-and-why-now with a decay check (is this a durable exposure or a narrow product launched near the hype). Claude structures the comparison, checks every fact is sourced, flags the traps, and gives a verdict; it does not fetch fund data, pick a theme, or decide how much goes to the sleeve. Not `asset-allocation-policy` — that sets whether a sleeve exists and how big it is; this picks the fund that fills it. Not `equity-research-writeup` — that is the individual-stock thesis writeup; this is its funds counterpart. Not `portfolio-thesis-audit` — that grades held individual positions on their thesis; a held broad-core index fund has none by design, a held thematic fund's tilt thesis is checked here. Not individual-bond selection, a robo-advisor or whole-managed-portfolio choice, or in-depth mutual-fund-vs-ETF tax mechanics (flag and name a CPA). Not writing options on an ETF you hold — that's `covered-call-decision`. Not a bare conceptual question — "what is an expense ratio", "how do ETFs work", "physical vs synthetic" with no fund on the table is `learning-gate`. Not a substitute for a financial advisor, and Claude's recollection of a fund's expense ratio or holdings is a prompt to check the fact sheet, never the figure of record.
---
