...
description: |-
  Decide a specific stock trade (real ticker, entry, capital): enter or not, share count, size sanity. Triggers: "should I buy X here", "how many shares", "is this position too big", "what's my risk on this trade". Forces in sequence: a pre-trade checklist (fundamentals, trend, relative strength, entry timing) before a share count; a cycle stage sourced from `macro-cycle-read`, never asserted from a vibe, before a tilt; position size as risk-budget divided by per-share-risk, kept separate from position cost; and a reward:risk ratio (target minus entry, over entry minus stop) checked against a stated minimum before the size is called good. Not options, day-trading, or leveraged income (covered calls: `covered-call-decision`, enter here first). Not allocation policy (`asset-allocation-policy` sets the bands), not keep/exit of a held position (`portfolio-thesis-audit`), not its stop (`position-exit-rules`), not tip pre-screening (`watchlist-screener-criteria`), not backtest sizing (`strategy-backtest-design`).
---
