...
- **Options, day-trading, or leveraged/aggressive income strategies.** Different risk math
  ...
  and stop rather than force this skill's checklist onto a trade shape it wasn't built for. A
  covered-call income plan on a long-equity position is `covered-call-decision`; this skill
  sizes the entry, then hands off.
...
- **Review a position already held.** Whether an existing holding still has a live thesis, or
...
1. **Fundamentals.** Actually reviewed the latest annual report / 10-K, not assumed sound.
   ...
   this catalog. The structured version of this review is `equity-research-writeup` (the
   8-section thesis writeup); a completed writeup satisfies this item, and "I read it, looks
   solid" with no stated takeaway does not. If the name arrived by narrative (a video, a tip,
   "I use the product") and never cleared a numeric screen, run `watchlist-screener-criteria`
   before either — it's the gate before this gate.
...
- A narrative-sourced name (video / tip / "I use the product") sized here with no numeric
  screen ever run against it — that's `watchlist-screener-criteria`, the gate before this one.
