...
description: |-
  Review positions already held — keep, sell, or fix — against their thesis, not a new trade. Triggers: "which of these should I sell", "review my portfolio", "audit my holdings", pasting tickers with P&L. "Research my X position" when X is already held starts here, not `equity-research-writeup`. For each position: no keep/sell verdict until the user supplies three things — the written falsifiable thesis, whether it is still valid and on what evidence, and a specific exit condition (routes to `position-exit-rules` if missing). Claude stress-tests what the user brings, assigns hold / exit / research-required, and flags behavioral patterns (disposition effect, luck read as skill, tip-sourced thesis, averaging down) as hypotheses. Not entering or sizing a position (`equity-trade-decision`), not options positions, not writing calls on a survivor (`covered-call-decision`), not allocation policy (`asset-allocation-policy`), not the scheduled pass (`weekly-portfolio-review`).
---
