...
description: |-
  Set or revise the target shape of a whole portfolio — the policy every single-position decision operates inside. Triggers: "set a target allocation", "what's my stock/bond/cash split", "how big can one position be", "when do I rebalance", "am I too concentrated". A gate: no targets off a risk-tolerance quiz until the user supplies the capital map, the withdrawal plan and horizon, risk capacity vs. tolerance, a concentrated-vs-diversified stance, and what each bucket is for. Then the user commits the full policy — class targets with bands, equity sub-split, sizing bands, diversification limits, rebalancing trigger, contribution routing — and Claude checks it is complete, consistent, and stress-tested. Sets the bands `equity-trade-decision` sizes inside and `weekly-portfolio-review` checks drift against. Not per-position exits or keep/sell, not which names to research, not retirement contribution sequencing (`retirement-contribution-sequencing`), not a bare-concept question (`learning-gate`).
---
...
- **Optimise tax-advantaged contributions.** Backdoor Roth mechanics, capturing an employer
  match, the order to fund accounts — that's `retirement-contribution-sequencing`. This policy
  places the accounts in the capital map and routes contributions to an asset class once
  they're inside one; it doesn't decide which account/tax-wrapper gets the next dollar first.
