...
| Reviewing or auditing positions already held — which to keep, which to sell, which need research | State, per position, the written thesis, whether it still holds and on what evidence, and the specific exit condition | `portfolio-thesis-audit` (a gate — cede the precondition questions; distinct from `equity-trade-decision`, which sizes a *new* entry) |
| Defining a real position's exit rules before entry (or for a held position that never had them) — the price stop, the thesis-invalidating events, the size ceiling | State the single price-stop number, each thesis clause + its one invalidating event, and the size-cap % | `position-exit-rules` (a gate — cede the precondition questions; distinct from `equity-trade-decision`, which sizes, and `portfolio-thesis-audit`, which judges keep/sell) |
| Writing covered calls against a held 100+ share lot for premium income — strike, expiration, the assignment tradeoff | State the position, the price target, honest willingness to be assigned, and whether the options chain is liquid | `covered-call-decision` (a gate — cede the precondition questions; downstream of `portfolio-thesis-audit`, consumes `position-exit-rules`' price target as the strike floor) |
| Building a numeric stock screen, or wanting a repeatable candidate process instead of tips and videos — the style, the criteria (metric + threshold + direction + why), the disqualifiers | Name the style + one criterion's threshold with the mechanism behind it | `watchlist-screener-criteria` (a gate — cede the precondition questions; Define builds the screen, Screen runs one candidate; upstream of `equity-trade-decision`, which sizes a name that already cleared it) |
| Running the recurring operational pass over a brokerage account — the weekly/periodic review walk, "what do I look at this week" | State the holdings table, the thesis/exit-rules status per position, any open covered calls, the forward calendar, and the last review date | `weekly-portfolio-review` (a procedure, not a rep gate — cede the coaching level; the integration pass over the other Finance skills, distinct from `portfolio-thesis-audit`, the per-position keep/sell audit it flags positions for) |
