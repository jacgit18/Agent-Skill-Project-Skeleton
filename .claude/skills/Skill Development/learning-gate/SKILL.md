...
| Architecture decision | Name constraints + a lean | `problem-solving-gates` (Options Generator), `database-architecture`, `microservices-decision` (service boundaries, repo layout, frontend decomposition), `bff-gateway-placement` (gateway/BFF topology, cross-cutting-concern placement), `service-mesh-adoption` (whether a service mesh exists for service-to-service traffic, and discovery mechanism), `api-interface-style` (API protocol / interaction model), `observability-strategy` (how a system is instrumented), `resilience-strategy` (overload / failure protection), `migration-cutover` (moving a live workload to a new system), `deployment-strategy` (how a version rolls out), `technical-cost-decision`, `cloud-iam-boundary` (who/what gets access, and network placement), `access-control-modeling` (which end user may do what to which resource, and how tenants are isolated), `serverless-execution-model` (compute primitive, invocation model, orchestration) |
...
- Run this skill's rep gate on top of a domain skill's own gate when both match the request. Classify the intent, then hand off — the domain skill (`database-architecture`, `relational-modeling`, `dimensional-modeling`, `data-tier-operations`, `caching-strategy`, `observability-strategy`, `resilience-strategy`, `capacity-estimation`, `failure-mode-analysis`, `design-scoping`, `migration-cutover`, `deployment-strategy`, `api-interface-style`, `microservices-decision`, `bff-gateway-placement`, `service-mesh-adoption`, `technical-cost-decision`, `cloud-iam-boundary`, `access-control-modeling`, `serverless-execution-model`, `test-strategy`, `coverage-policy`, `test-practice-gate`, `problem-solving-gates`, `code-review`) owns the precondition questions.
