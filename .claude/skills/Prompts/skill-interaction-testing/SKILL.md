...
description: Use after writing or substantially revising a skill in .claude/skills/, before calling it done — tests it against the siblings it could realistically collide with (its own group, a fixed short list of cross-cutting gates, plus any out-of-group skill a quick scan flags as sharing a concrete concept; Step 1 scopes the pool, never the full catalog) for stacking (gates firing in sequence), contradiction (opposite instructions), silent override (a broader skill claims the request first), or beneficial chaining (one output feeding another's input). Complements writing-skills, which only tests a skill in isolation. Also use when an existing skill's trigger description is edited in a way that could newly overlap another's. Skip with fewer than two skills, and for edits that don't change triggering (typos, polish, examples). Not the periodic whole-catalog hygiene pass — that's catalog-drift-audit, which hands flagged pairs back to this skill's Step 2 onward.
---
...
2. **The fixed cross-cutting list.** Always read these regardless of the new skill's group, because their triggers are about request *shape*, not subject matter, so they can fire alongside a skill from any domain: `ambiguity-gate`, `learning-gate`, `problem-solving-gates`, `spec-drift-gate` — matching `.claude/rules/skill-architecture.md`'s "four cross-cutting meta-skills" list, which is canonical. (Keep this list itself short — a skill only belongs on it if its description says it triggers on how a request is phrased or where it sits in a workflow, not on what domain it's about. Add to it only when a new skill earns that description; if this list ever grows past ~6-8 entries, its own trigger shapes have probably drifted apart and it should be split, not just extended. `problem-journal` was on this list previously — moved off, since its trigger is domain-specific (coding-error capture/journal), not shape-based; check it as an out-of-group tier-3 candidate instead when a new skill's concept actually overlaps.)
...
Example: a new `Finance/` skill's pool is every other `Finance/` skill (tier 1) + `ambiguity-gate`, `learning-gate`, `problem-solving-gates`, `spec-drift-gate` (tier 2) + `Business/technical-cost-decision` (tier 3 — both are about pricing a recurring decision, a concrete shared concept, and prior Finance skills already hand off to it) + `Prompts/problem-journal` when the new skill's own concept plausibly recurs as a trackable mistake pattern (checked, not assumed — several Finance skills have confirmed clean against it this way). It does not include `Git/commit-and-push`, `Documents/document-page-check`, or most of `Architecture/` — nothing in their one-line descriptions shares a concept with a money decision, so they're never even fully read.
