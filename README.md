...
| [`Finance/watchlist-screen-sync`](.claude/skills/Finance/watchlist-screen-sync/) | Wraps `watchlist-screener-criteria` Screen mode and adds an automatic filing step: a candidate that clears the user's written screen gets added to a real Webull "Screen Passed" watchlist, a fail to "Screen Rejected" — only when the user's phrasing signals they want the outcome actually tracked in Webull, not just reported in chat. Still runs the full screen-result block first (no shortcut for "just file it"), checks both lists for an existing entry before filing to avoid duplicates or a silent re-file, and flags any metric value sourced only from a video/tip as unverified rather than running it as sourced. Not screen-definition, ETFs, trade sizing, the full thesis writeup, or a no-ticker "what order" ask — those stay with `watchlist-screener-criteria` Define mode, `etf-selection`, `equity-trade-decision`, `equity-research-writeup`, and `stock-review-order` respectively. The one Finance skill that isn't repo-agnostic — it hardcodes two watchlist IDs specific to one Webull account. |
...
watchlist-screen-sync        files a Screen verdict into a real Webull watchlist      (side-effect)
