...
| [`Prompts/problem-journal`](.claude/skills/Prompts/problem-journal/) | Journals a resolved coding problem — symptom/cause/fix, a recurrence count grepped from prompt-archive's dated logs, and a worth-learning verdict that must name the count behind it. Hands teaching back to `learning-gate`/`problem-solving-gates` rather than doing it inline. |
