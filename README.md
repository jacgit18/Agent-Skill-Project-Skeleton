...
| [`Finance/equity-trade-decision`](.claude/skills/Finance/equity-trade-decision/) | Forces a pre-trade checklist, an evidence-backed economic-cycle-stage call, a position size computed as risk-budget ÷ per-share-risk (kept separate from money actually spent), and a reward:risk ratio checked against a 2:1 floor, before sizing or entering a real stock trade. |
...
| [`Finance/weekly-portfolio-review`](.claude/skills/Finance/weekly-portfolio-review/) | **Procedure**, not a gate — the recurring integration pass over the other Finance skills. A fixed seven-step weekly walk (positions vs. thesis, stops still valid, options expiring, earnings/events ahead, allocation drift vs. size ceiling — including a live check for any held ticker sitting on 2+ `watchlist-screen-sync` style lists at once, flagged as one concentrated bet wearing several labels — watchlist in range, since-last-review delta) that **surfaces and routes** each finding to the skill that owns the decision (`portfolio-thesis-audit` / `position-exit-rules` / `covered-call-decision` / `equity-trade-decision` / a trim flag) rather than deciding inline. Runs on a calendar cadence on purpose. Includes an automation-boundary table (script does the data, human does the judgment). Distinct from `portfolio-thesis-audit` — that's the per-position keep/sell audit this pass flags positions for. |
| [`Finance/performance-benchmarking`](.claude/skills/Finance/performance-benchmarking/) | **Procedure** for the one question nothing else in the catalog asks: did the whole account actually beat a plain index fund, risk-adjusted, over real time. Withholds a verdict until start/end value+date and every external cash flow between them are supplied — computes return via the Modified Dietz method, fetches the benchmark's real price for the same window via Webull, and reports the delta in percentage points, never the portfolio return alone. Computes Sharpe/Sortino only with ≥3 periodic snapshots on file; says so and skips otherwise rather than faking a ratio from too few points. Appends every run to a running log (`Finance/Investment/Performance Log.md` in the vault) so a real return series builds over time. Distinct from `weekly-portfolio-review` (process, not realized results), `strategy-backtest-design` (a hypothetical rule vs. real account history), and `equity-trade-decision`'s single-stock relative-performance checklist line. |
| [`Finance/trading-decision-journal`](.claude/skills/Finance/trading-decision-journal/) | Two modes. **Capture** — the moment a real buy/sell/add/trim happens, logs it verbatim (ticker, the user's own stated reason, sourced-vs-unsourced, and for an exit whether the stop was honored or moved) to `Finance/Investment/Trading Decision Log.md` in the vault; never judges, never blocks the decision. **Review** — a periodic pass over the log flagging only patterns the data actually supports: rising stop-override rate, thesis rewrites with no new evidence, a drift toward unsourced decisions, frequency spikes, asymmetric win/loss detail — "not enough entries yet" is a valid answer below ~5 in the window. Checks the user's *behavior*, not the thesis (`portfolio-thesis-audit`) and not a coding error (`problem-journal`, a different domain entirely). |
| [`Finance/equity-research-writeup`](.claude/skills/Finance/equity-research-writeup/) | Gate that withholds a finished single-company research writeup — and refuses to author the analysis — until the user fills eight sections in their own words with sourced figures: the business, unit economics, competitive-position *evidence*, 3–5y financial trend, valuation reasoning, their own bear case, a falsifiable ≥3-sentence thesis, and what-would-change-my-mind. Claude supplies the structure, checks every claim has a figure and a named source, pushes on a weak bear case, and flags stale-from-memory financials — it never writes a section, supplies a figure, or runs a valuation. Two delivery modes: **Direct** (bare skeleton) and **Coach** (for someone new to reading fundamentals — pulls sourced figures via `equity-research-sourcing` first, teaches what each pattern generally means, asks a guiding question per section; still never asserts the verdict for this company — that stays `equity-investment-take`'s job, a different skill). Sits between `watchlist-screener-criteria` and `position-exit-rules` / `equity-trade-decision`; section 7 satisfies the trade checklist's fundamentals item and section 8 becomes `position-exit-rules`' thesis-invalidating events. Not for ETFs. |
| [`Finance/equity-research-sourcing`](.claude/skills/Finance/equity-research-sourcing/) | Fetch-and-cite companion to `equity-research-writeup`, closing the gap between "the user must verify everything themselves" and "hand-copying a filing one field at a time doesn't protect anyone." Three modes: **Fetch** — pulls and cites the objective figures behind sections 2/4/5 (unit economics, 3–5y financial trend, valuation) live via Webull financial-data tools, flagging any gap `NOT VERIFIED` rather than estimating it. **Lookup** — points to where a figure lives for the user to pull themselves. **Scope** — narrows an open-ended qualitative ask ("research the competitive position") to one checkable claim before anything runs, then either executes one targeted search or hands back a precise query for the user to run elsewhere (e.g. ChatGPT) and paste back, vetting every returned claim for a named, dated source before it counts as evidence. Never writes or interprets a section — the business explanation, competitive argument, bear case, thesis, and invalidation events (sections 1, 3, 6, 7, 8) stay entirely the user's own synthesis; also feeds `portfolio-thesis-audit`'s current-validity check. Not `watchlist-screener-criteria` Screen mode (values stay user-supplied by design), not `reddit-researcher` (a broad sentiment sweep vs. one verified claim), not ETF fact-sheet data. |
| [`Finance/equity-investment-take`](.claude/skills/Finance/equity-investment-take/) | The one deliberate exception to the catalog's "Claude never authors the investment case" rule — a separate, explicitly-invoked skill (not `equity-research-writeup`'s gate released under pressure) for when the user genuinely wants Claude's own sourced opinion instead of doing the section-by-section work themselves. Fetches real data live with the same tools and sourcing bar as `equity-research-sourcing`, then commits to an actual verdict — business, key numbers, valuation, competitive position and bear-case evidence (including any adverse/short-seller reports), structural risk factors, a "consider/watch" list, and a stated bottom-line read with reasoning, never a hedge. `equity-research-writeup`'s own gate still holds under pressure ("just give me the analysis") and now names this skill as the real door instead of a dead end — the user still has to ask for it directly. Not the default for "research TICKER"/"deep dive," not ETFs (`etf-investment-take`), not a held position (`portfolio-thesis-audit` — ownership language keeps it there even when phrased as "what do you think"), not sizing or entering the trade (`equity-trade-decision`). |
| [`Finance/etf-investment-take`](.claude/skills/Finance/etf-investment-take/) | The funds counterpart to `equity-investment-take`, same parallel-door relationship to `etf-selection`'s gate (unchanged, now names this skill under pressure the same way `equity-research-writeup` does). Fetches real fund data live via Webull's fund-data tools plus the issuer fact sheet, then gives a sourced BUY/HOLD/SWAP/PASS/RESEARCH verdict — expense ratio, tracking, structure, holdings overlap (asked for once if not supplied), a Claude-authored tilt thesis + decay check for a non-core fund, a "consider/watch" list, and real reasoning, never a hedge. Unlike the stock side's new/held split, `etf-selection` keeps a held fund's re-check internal rather than routing to `portfolio-thesis-audit`, so this skill is the opinion-door for both a new fund and a held one. Not stocks (`equity-investment-take`), not the sleeve decision (`asset-allocation-policy`), not sizing/entering (`equity-trade-decision`), not options on a held fund (`covered-call-decision`). |
...
| [`Finance/rebalancing-execution`](.claude/skills/Finance/rebalancing-execution/) | Turns an already-flagged allocation drift into an actual trim/buy plan — the step `asset-allocation-policy` names but doesn't do, and `weekly-portfolio-review` flags but doesn't execute. Applies the policy's own already-set tax-aware order mechanically (new money + dividends first → tax-advantaged accounts → taxable only past the stated threshold), computes real dollar/share amounts, and hands a confirmed taxable sale to `tax-lot-selection` for the specific lot. The critical guardrail: a breach must be confirmed as a thesis-intact rebalance before proceeding — a breach from a broken thesis routes to `portfolio-thesis-audit` / `position-exit-rules` instead. |
| [`Finance/lump-sum-vs-dca`](.claude/skills/Finance/lump-sum-vs-dca/) | For a real, consequential one-time sum (bonus, inheritance, sale proceeds) already destined for an existing `asset-allocation-policy`, decides the entry schedule using real historical price data pulled live via Webull — never a recited "studies show" statistic. Computes the actual lump-sum-vs-staged win rate across real rolling windows for the destination benchmark, then presents options against the user's own stated regret-risk tolerance; Claude states what the evidence favors, the schedule itself is the user's call. Not for routine small recurring contributions — those stay the policy's own generic cadence default. |
| [`Finance/portfolio-dashboard-calculator`](.claude/skills/Finance/portfolio-dashboard-calculator/) | **Not a gate — the arithmetic layer.** Runs the dashboard's math from chat via `scripts/calc.py` (split to the cent, drift, held, stress, monthly, trade sizing, ticker ranking); never a verdict. Same risk-budget math as `equity-trade-decision`; drift has no tax ordering (`rebalancing-execution`). |
...
| [`Finance/debt-credit-management`](.claude/skills/Finance/debt-credit-management/) | Gate for a real credit-card decision — close/consolidate/keep, payoff order, "too many cards." Withholds a verdict until every card's APR, balance, limit, annual fee, rewards value, and account age is on the table, combined utilization (total balances ÷ total limits) is computed across all cards, and the user names one goal (pay down fastest / protect score / simplify). Forces the before/after combined-utilization and average-account-age effect of any proposed close — the trap where "cutting a card you don't use" quietly raises utilization and shortens history. Names, but doesn't resolve, the capital-routing tension between a high-APR balance and `asset-allocation-policy`'s runway/contribution targets. Revolving credit only — installment debt (mortgage, auto, student loans) is out of scope. |
| [`Finance/tax-lot-selection`](.claude/skills/Finance/tax-lot-selection/) | Gate for *which* lot to sell once a sale is already decided — by `position-exit-rules`, `portfolio-thesis-audit`, `rebalancing-execution`, or a `covered-call-decision` assignment — in a taxable account. Withholds an answer until every lot (acquisition date, quantity, cost basis) is on the table and one goal is named (minimize this year's tax / avoid short-term treatment / broker default). Computes each lot's holding period and gain/loss, cross-checks against what the broker's default method would actually pick, and runs a wash-sale check — including the dividend-reinvestment trigger — before confirming any loss sale. Taxable accounts only; a Roth/IRA/HSA sale has no lot to track. Never decides whether to sell, only which shares once that call is made. |
| [`Finance/retirement-contribution-sequencing`](.claude/skills/Finance/retirement-contribution-sequencing/) | Gate for *which account* gets new retirement dollars first — 401(k) match, HSA, Roth (direct or backdoor), rollover IRA, taxable. Withholds a funding-order answer until every account's contributions-so-far and limit-remaining is on the table, the employer match formula and capture status is stated, HSA/HDHP eligibility is confirmed (not assumed), and — near the Roth income limit — the existing pre-tax IRA balance is checked for the backdoor-Roth pro-rata trap. Applies the standard hierarchy (full match → HSA → Roth → rest of 401(k) → taxable) against real numbers, never recited generically. Upstream of `asset-allocation-policy`'s contribution routing (this picks the account, that picks the asset class inside it); names, doesn't resolve, the competing claim from `debt-credit-management`. |
| [`Finance/strategy-backtest-design`](.claude/skills/Finance/strategy-backtest-design/) | Gate for backtesting a rule-based trading strategy on real historical price data before capital is risked — not a live-trade recommendation. Withholds the run until ticker(s), a testable entry rule, a paired exit rule (stop-loss + trend/target), timeframe/lookback, and a sizing assumption are stated; offers a literature-sourced menu (Van Tharp / Turtle / fractional-Kelly sizing, Minervini / golden-cross / Connors RSI(2) entries, ATR-chandelier / death-cross / RSI exits) rather than inventing the logic. Pulls real daily bars via the Webull tool and walks the rule bar-by-bar; every output states the trade count against the ~100-trade reliability floor, an explicit buy-and-hold comparison, and single-stock fragility. Not sizing/entering a live trade (`equity-trade-decision`) or exit rules for a held position (`position-exit-rules`); options strategies, formal walk-forward validation, and multi-asset backtests are named gaps, not built. |
| [`Finance/macro-cycle-read`](.claude/skills/Finance/macro-cycle-read/) | Gate that calls which economic-cycle stage we're in — post-recession recovery, recovery momentum, mid-cycle expansion, late-cycle/peak, or recession/contraction, the same five names `equity-trade-decision`'s sector-tilt table uses — from real, current, sourced values, never vibes. Withholds a stage call until ≥3 of 5 evidence categories are pulled live via search/fetch (yield curve, labor market, leading activity/ISM-PMI, credit spreads, inflation trajectory), names the stage as a hypothesis, and states explicitly where indicators conflict. Feeds `equity-trade-decision`'s cycle-stage step; never picks a sector/stock/ETF itself and never licenses moving outside `asset-allocation-policy`'s bands or a `strategy-backtest-design` macro-regime run (not built). |
| [`Finance/stock-review-order`](.claude/skills/Finance/stock-review-order/) | **Not a gate** — a quick reference. When no ticker is on the table yet, returns the fixed eight-step checklist for the order to examine a stock (numeric pre-screen → business in plain terms → unit economics → competitive position → 3–5yr financial trend → valuation → bear case → thesis + invalidation events), one line per step, with no forced sections and no sourced-figure requirement. Step 1 points to `watchlist-screener-criteria`; the deep steps mirror `equity-research-writeup`'s eight sections, which owns doing the work for real once a ticker is named with research intent — this skill hands off there rather than answering inline. |
| [`Finance/watchlist-screen-sync`](.claude/skills/Finance/watchlist-screen-sync/) | Wraps `watchlist-screener-criteria` Screen mode and files a clearing candidate to a real Webull "Screen Passed" watchlist (a fail to "Screen Rejected") — plus five committed style screens (Deep Value, Quality Compounder, Growth, Momentum, Dividend Income), each with its own hardcoded criteria table and its own "AI [Style]" watchlist; a ticker can land on more than one if it clears more than one style. Fires only when the user's phrasing signals they want the outcome actually tracked in Webull, not just reported in chat. Runs the full screen-result table first (no shortcut for "just file it"), checks every target watchlist for an existing entry before adding (a ticker can trigger several adds in one run), and flags any metric value sourced only from a video/tip as unverified rather than running it as sourced. Never loosens or reinterprets a criterion — that needs the user's explicit sign-off, same bar as any other screen edit. Has a separate style-watchlist re-check pass, distinct from `weekly-portfolio-review`'s whole-account walk; that pass also resyncs **AI Shortlist**, a cross-style aggregator with no criteria of its own — it mirrors each style's top-2-ranked passer(s), adds are automatic, demotions are flagged only. **AI Day Trade** is a manual-only liquidity/volatility screen (price floor, dollar volume, relative volume, ATR%) — deliberately excluded from every unattended pass, since intraday setups go stale within hours. **Candidate discovery mode**: the one place in the catalog Claude sources tickers itself — pulls a raw candidate pool per style from Webull's real market-scan tools (strong fit for Dividend Income/Growth/Momentum; for Deep Value/Quality Compounder, the unattended run samples one rotating batch of 9 Webull sectors per run — chosen statelessly by ISO week number — so the full sector universe is covered over several weeks and the "AI" in each watchlist's name, a list-ownership marker only, never silently collapses discovery into an AI/semiconductor screener), fully evaluates at most 5 new candidates per style per run, retrieves financials progressively (cheapest disqualifying endpoint first, early exit, run-wide cache, existing holdings reviewed before any discovery, and it degrades by stopping rather than by weakening a screen), runs the same criteria tables, and auto-files passers; Momentum's cross-sectional top-30% tests report `data not available` rather than guess when the discovery universe is too narrow; Day Trade has its own manual-only discovery variant sourced from intraday movers/volume scanners. Not screen-definition, ETFs, trade sizing, the full thesis writeup, or a no-ticker "what order" ask — those stay with `watchlist-screener-criteria` Define mode, `etf-selection`, `equity-trade-decision`, `equity-research-writeup`, and `stock-review-order` respectively. The one Finance skill that isn't repo-agnostic — it hardcodes nine watchlist IDs specific to one Webull account. |

```
Equity pipeline — each stage a gate, running inside the one above:

stock-review-order           memory aid for the order below, not a stage itself       (reference)
watchlist-screen-sync        files a Screen/style verdict into real Webull watchlists (side-effect)
retirement-contribution-sequencing  which ACCOUNT gets a new dollar first (match/HSA/Roth/etc.) (gate)
asset-allocation-policy      whole-book target shape — the frame the rest operates inside     (gate)
  rebalancing-execution      turns a flagged drift into a real trim/buy plan                  (execution)
  lump-sum-vs-dca            entry schedule for a real one-time sum, real historical evidence  (procedure)
  watchlist-screener-criteria  numeric pre-filter: does a name earn research?   Define/Screen  (gate)
    equity-research-writeup    8-section thesis writeup for one company                        (gate)
      equity-research-sourcing   fetches/cites figures + scopes qualitative claims for it      (companion)
      equity-investment-take     parallel door: Claude's own sourced verdict, on request       (opinion)
    etf-selection              fact-sheet evaluation for one fund                              (gate)
      etf-investment-take        parallel door: Claude's own sourced fund verdict, on request  (opinion)
      position-exit-rules      price stop + invalidating events + size cap, before entry      (gate)
        equity-trade-decision  size and enter, inside the allocation bands                    (gate)
          macro-cycle-read     sources the cycle stage equity-trade-decision's tilt consumes  (gate)
...
trading-decision-journal     logs each decision, later flags process/behavior drift          (procedure)
performance-benchmarking     whole-account realized return vs. a real benchmark, periodic     (procedure)

seller-financing-evaluation  a seller-financed *business* purchase — not equities             (gate)
debt-credit-management       revolving credit-card debt — the liability side, not equities   (gate)
tax-lot-selection            which lot, once ANY of the sell-producing gates above fires      (gate)
```
...
| [`Skill Development/incremental-build-pacing`](.claude/skills/Skill%20Development/incremental-build-pacing/) | Delivery-cadence gate for AI-assisted implementation of an **already-planned** build the user wants to follow as it's assembled — withholds the large one-turn batch, sets an increment size + checkpoint cadence, and delivers one piece at a time with a comprehension check before each advance. Governs cadence, not scope: `spec-drift-gate` / `design-scoping` settle the slice first; `learning-gate` Step 3 routes here once intent is "understand a build as it's assembled". |
...
| [`Architecture/tech-decision-walkthrough`](.claude/skills/Architecture/tech-decision-walkthrough/) | Coached, conversational **procedure** for choosing a build's technologies out loud — one decision at a time, system-design-interview style, with alternatives / discriminating axes / honest tradeoffs made explicit, ending each in a recommendation, the user's call, and a short ADR. Depth scales with blast radius; load-bearing decisions route into the specialist Architecture skill and come back as an ADR. Downstream of `design-scoping` (which produces the decision list), upstream of `spec-drift-gate` → `incremental-build-pacing`. Register (collaborative vs interviewer) set by `learning-gate`'s assistance level. Not `problem-solving-gates` Options Generator (which withholds Claude's list until the user brings candidates). |
